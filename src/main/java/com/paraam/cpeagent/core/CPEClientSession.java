...
public class CPEClientSession {
    ...
    public CPEClientSession(CpeActions cpeActions, String acsurl, String username, String passwd, String authtype,
            String useragent, XmlFormatter xmlFmt, boolean strangeAcs) {
        ...
        System.out.println("ACS MGMT URL -------> " + acsurl);
        service = ResourceAPI.getInstance().getResourceAPI(acsurl);
    ...
    public void sendInform(Envelope envelope) {
        ...
        // System.out.println("Received InformResponse Max Envelopes ===== " +
        // iresp.getMaxEnvelopes());
        ...
        String serial = ((ConfParameter) this.cpeActions.confdb.confs
                .get(this.cpeActions.confdb.props.getProperty("SerialNumber"))).value;
    ...
    public void handleACSRequest(ACSResponse acsresp) {
        ...
        if (response != null && response.length() > 0) {
            ...
            try {
                ...
                String sn = ((ConfParameter) cpeActions.confdb.confs
                        .get(cpeActions.confdb.props.getProperty("SerialNumber"))).value;
            ...
            Envelope envResp = getClientResponse(cpeActions, idObj, reqobj, this.strangeAcs);
            String respBody;
            if (this.xmlFmt == null) {
    ...
    private ACSResponse sendData(WebResource service, String reqString) {
        ...
        for (NewCookie c : cookies) {
            // System.out.println( "Request Setting cookie ======================== " +
    ...
    public Envelope getClientResponse(CpeActions cpeactions, Object idobject, Object reqobject, boolean strangeAcs) {
        ...
        switch (switchId) {
            ...
            case ClientUtil.SCHEDULE_INFORM_ID:
                ...
                SchedulerInform siclass = new SchedulerInform(delaysec, acsurl, username, passwd, authtype, useragent,
                        xmlFmt);
    ...
    public void dumpCurrentConfiguration(String dumploc, String serial) {
        try {
            ...
            Envelope envLearnedValues = cpeActions.doGetParameterValues(learnedParameterValues, false,
                    cpeActions.confdb.learns);
            String learnedValuesDump = JibxHelper.marshalObject(envLearnedValues, "cwmp_1_0");
            CpeDBReader.serialize(dumploc + "learnedvalues_" + serial + ".xml",
                    new XmlFormatter().format(learnedValuesDump));
     ...
     * return
     * "<soap:Header><cwmp:ID soap:mustUnderstand=\"1\">100110</cwmp:ID></soap:Header>"
     * ;
     * }
     * 
     ...
     * Element node = null;
     * String informString =
     * "<soap:Body><cwmp:Inform><DeviceId><Manufacturer>Conexant</Manufacturer><OUI>002615</OUI><ProductClass>ADSL2+ IAD</ProductClass>"
     * +
     * "<SerialNumber>00261559a496</SerialNumber></DeviceId><Event soapenc:arrayType=\"cwmp:EventStruct[1]\">"
     * +
     * "<EventStruct><EventCode>4 VALUE CHANGE</EventCode><CommandKey></CommandKey></EventStruct></Event>"
    ...
    public class SchedulerInform implements Runnable {
        ...
        public SchedulerInform(int delaysecs, String acsurl, String username, String passwd, String authtype,
                String useragent, XmlFormatter xmlFmt) {
        ...
        public void run() {
            try {
                ...
                try {
                    this.acsurl = ((ConfParameter) cpeActions.confdb.confs
                            .get(cpeActions.confdb.props.getProperty("MgmtServer_URL"))).value;
                    this.username = ((ConfParameter) cpeActions.confdb.confs
                            .get(cpeActions.confdb.props.getProperty("MgmtServer_Username"))).value;
                    this.passwd = ((ConfParameter) cpeActions.confdb.confs
                            .get(cpeActions.confdb.props.getProperty("MgmtServer_Password"))).value;
                } catch (NullPointerException missingProps) {
                    System.out.println(
                            " WARNING: ACS credential switch not abailable, missing property for MgmtServer_Username or MgmtServer_Password mapping in "
                                    + cpeActions.confdb.props.toString());
                }

                System.out.println("Sending ScheduleInform Message at " + (new Date()));
                CPEClientSession session = new CPEClientSession(cpeActions, acsurl, username, passwd, authtype,
                        useragent, xmlFmt, strangeAcs);
